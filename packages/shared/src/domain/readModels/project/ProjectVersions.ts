...
export type ProjectVersionEntry = {
  /** Semantic/version label from app metadata (`metadata.json` `version` field). Null when missing/blank. */
  version: string | null;
...
export const projectVersionEntrySchema = z.object({
  version: z
    ...
    .nullable()
    .describe(
      "Version string from project metadata. Null when the version field was missing or blank for that revision."
    ),
  latestRevision: z
    .number()
    .int()
